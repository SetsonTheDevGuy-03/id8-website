...
import { ArrowRight, Sparkles, Code2, Bot, Rocket, BarChart2 } from "lucide-react";
...
const cardData = [
  ...
  },
  {
    ...
    desc: "KPI dashboards and reporting systems that drive decisions.",
    icon: BarChart2,
...
const clients = [
  "Invest in Africa Expo",
  "NowNow Tours & Safaris",
  "DTS Football Club",
  "Coding Kids Robotics",
];
...
export function Hero() {
  ...
  useEffect(() => {
    ...
    if (isDeleting) {
      ...
      timer = setTimeout(() => { setCurrentText((prev) => fullWord.slice(0, prev.length + 1)); }, 100);
    }
  ...
  return (
    <section className="relative min-h-[95vh] flex flex-col justify-between pt-24 pb-12 sm:pt-28 md:pt-32 overflow-hidden bg-[#020204]">
      ...
      <div className="hidden lg:flex absolute left-8 top-1/3 bottom-12 flex-col items-center justify-between pointer-events-none select-none z-20">
        ...
        <span className="font-mono text-[10px] tracking-[0.25em] text-white/30 uppercase rotate-90 my-24 origin-center">Scroll</span>
        ...
        <div className="lg:col-span-6 flex flex-col items-start text-left space-y-6 md:space-y-8">
          ...
          <Reveal delay={0.1}>
            <p className="font-sans text-slate-350 text-base md:text-lg leading-relaxed max-w-xl">
              A Namibia-born digital systems studio delivering websites, software, AI automation, and brand systems that help businesses operate with confidence.
            </p>
          </Reveal>
          ...
          <Reveal delay={0.2}>
            <div className="flex flex-col gap-3 pt-4 border-t border-white/5 w-full">
              <span className="font-sans text-xs text-white/40 uppercase tracking-widest font-mono">Trusted by</span>
              <div className="flex flex-wrap gap-x-6 gap-y-2">
                {clients.map((client, i) => (
                  <span key={i} className="font-sans text-xs text-white/55 font-medium">{client}</span>
                ))}
              </div>
            </div>
          </Reveal>
              ...
              return (
                ...
                >
                  <div className="space-y-2.5">
                    ...
                    <div>
                      <h4 className="font-display font-medium text-sm text-white tracking-wide">{card.title}</h4>
                      <p className="font-sans text-xs text-white/50 mt-1 leading-relaxed">{card.desc}</p>
                    </div>
                  </div>
                  <span className="text-white/20 group-hover:text-cyan-400 group-hover:translate-x-1 group-hover:-translate-y-1 transition-all text-xs font-bold leading-none select-none">&rarr;</span>
                </Link>
              );
            })}
          </div>
